...
for i in range(1,N[0]):
	...
	DDM_hat_h = (alpha2-a1_hat[i-1])*DM_gamma[i-1]+(alpha1-a2_hat[i-1])*M_gamma[i-1]+b_hat[i]*S_gamma[i-1]-alpha2*DM_hat[i-1]-alpha1*M_hat[i-1]
...
for i in range(1,N[0]):
	DDM_pred = -a1_hat[0]*DM_pred[i]-a2_hat[0]*M_pred[i]+b_hat[0]*S_gamma[i]
	DDM_pred_h = -a1_hat[0]*DM_pred[i-1]-a2_hat[0]*M_pred[i-1]+b_hat[0]*S_gamma[i-1]
