...
#----------------------------------------------------------------------------------------------------------------
	def __init__(self):
		...
		self.FT_cam = 0
		self.x_ref = 170.0
		self.l = 30.0 #30 # Tamano de la recta con que se modela el camino
...
#----------------------------------------------------------------------------------------------------------------
	def callback_Cam(self,data_camera):
		...
		if (self.FT_cam==5):
			self.L = LKF.line_class_FT(150,list_px)
			self.FT_cam = self.FT_cam+1
		else:
			self.L,_ = LKF.get_lines(list_px,self.L,10.0)
		...
		mr = self.L[0,0]
		br = self.L[1,0]
		ey = 20.0-(y0-mr*x0-br)/np.sqrt(1+mr**2)
		e_psi = np.arctan(mr) 
		Ky =  0.45631688 
		Kpsi = 0.5265962 
