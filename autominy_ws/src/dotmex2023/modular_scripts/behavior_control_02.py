...
#																	LIDAR
	def callback_Lidar(self, data_lidar):
		...
		for r in data_lidar.ranges:
			...
			i = i+1
		"""
		self.R = np.clip(data_lidar.ranges,0.08,1.5)
		#self.R = np.put(self.R,[157,158,159,160,161,200,201,202,203,204],[rmax,rmax,rmax,rmax,rmax,rmax,rmax,rmax,rmax,rmax])
...
#																	CAMERA RGB
	def callback_Cam(self,data_camera):
		...
		lower = np.array([100,46,170])	# BGR: 0,45,146
		upper = np.array([140,255,255])	# BGR: 11,255,255
...
#																	YOLO
	def callback_Yolo(self,data_yolo):
		...
		for i in range(len(id_class)):
			...
			# Senales de transito
			#if (id_number == 15) and (area>=2167): self.tfRed = False 	# Sem Verde
			#if (id_number == 17) and (area>=2167): self.tfRed = True	# Sem Rojo
			if (id_number == 13) and (area>=2450): self.v_max = 175		# lim 50km_h
			if (id_number == 14) and (area>=1500): self.v_max = 225		# lim 100km_h
			...
			#if (id_number == 0) and (area>=4313) and (250<=cx<=380): self.Car = True			# Coche										# Carro
...
#******************************************************************************************************************
	def behavior_control(self):
		...
		if (self.tfRed==False): maneuver = 1
		else: maneuver = 0

		# Maneuver selector
		if (maneuver==0): self.stop_car()
		if (maneuver==1):
			print('--------SEGUIMIENTO DEL CARRIL-----------')
			self.lane_keeping(maneuver) # LANE KEEPING
		if (maneuver==2):
			print('--------EVASION-----------')
			self.lane_keeping(maneuver) # EVASION
...
#																	LANE KEEPING AND PASSING
	def lane_keeping(self,maneuver):
		...
		speed_msg.value = self.v_max #200
		y1 = 0
		y2 = 0
		...
		Ky = 0.5 #0.45631688 #0.48340796
		Kth = 0.65 #0.5265962 #0.53838659
		e_y = x1-self.x_ref
...
#																	PARKING
	def parking(self):
			...
			r0 = np.amin(R0)
			r180 = np.amin(self.R[162:199]) #np.amin(self.R[165:205])
			r90 = self.R[89]
