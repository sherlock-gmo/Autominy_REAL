...
path_libs = '/home/ros/dotMEX_Autominy_REAL/autominy_ws/src/dotmex2022/scripts_final/libs'
import sys
	...
	def maneuver(self):
		...
		#_______________________________________________________________________________________
			...
			if (self.step == 2):
				u = -1.0
				v = -Nv*0.85
				if (self.Dyaw>=0.0) or (r180<=0.325): #0.375
					self.step = self.step + 1
					self.delay_time()
			...
			if (self.step == 3):
				u = 1.0
				v = Nv
				if (r0<=0.2) or (self.Dyaw>=0.0) : #0.25
					self.step = self.step + 1
