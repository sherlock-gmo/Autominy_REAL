...
#																	LIDAR
	def callback_Lidar(self, data_lidar):
		...
		rmin = np.min(self.R)
		if (Rl==False) and (rmin>=0.4) and (self.Obj==False):
			self.Ev = False
			self.FT = 0
		#if (r225<=0.5) and (self.Ev==True):
		#	print('Regreso')
		#	self.Ev = False
		#	self.FT = 0
		#print(self.Obj)
...
if __name__ == '__main__':
	print("Nodo inicializado: simple_passing_04.py")
	rospy.init_node('simple_passing_04',anonymous=True)
	autominy()
	rospy.spin()
