...
#																	LIDAR
	def callback_Lidar(self, data_lidar):
		...
		for r in data_lidar.ranges:
			if (r>=rmax): r = rmax
			if (i>=159) and (i<=161): r = rmax
			if (i>=219) and (i<=221): r = rmax
			self.R[i] = r
			i = i+1
...
#																	IMU
	def callback_Imu(self, data_imu):
		...
		if (self.yaw0>=360.0-self.D*1.3) and (yaw<=self.D): yaw = yaw+360.0
		if (self.yaw0<=self.D) and (yaw>=360-self.D*1.3): yaw = yaw-360.0
...
#----------------------------------------------------------------------------------------------------------------
	def maneuver(self):
		...
		#_______________________________________________________________________________________
			...
			r0 = np.amin(R0)
			r180 = np.amin(self.R[159:199])
			r90 = self.R[89]
			r_alin = self.R[140] #135
			if (self.step == 0):
				rmax = 3.0
				rmin = 0.10
			...
			if (self.step == 2):
				u = -1.0
				v = -Nv
				if (self.Dyaw>=0.0) or (r180<=0.35): self.step = self.step + 1
			
			if (self.step == 3):
				if (self.Dyaw<0.0): u = 1
				if (self.Dyaw>=0.0): u = 0
				v = Nv
				if (r0<=0.35) or (self.Dyaw>=0.0) :
					self.step = self.step + 1
			
			if (self.step == 4):
				#v = 0
				#u = 0
				if (self.Dyaw<0.0): self.step = 2
				if (self.Dyaw>=0.0):
