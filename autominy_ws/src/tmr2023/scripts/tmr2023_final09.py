...
#																PERPENDICULAR PARKING
	def perpendicular_parking(self):
			...
			if (self.step == 0):
				...
				flag_space = lidar_roi_f(self.R,self.side, depth)
				print(flag_space)
				if (flag_space == False) and (self.car_roi==False):
					self.car_roi = True
					self.stop_car_p()
				if (self.car_roi== True) and (flag_space==True):
					self.second_car = True
					self.stop_car_p()
				#if (self.second_car==True): flag_space2 = lidar_roi_f2(self.R,self.side)
