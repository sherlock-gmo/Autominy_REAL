...
#******************************************************************************************************************
	def behavior_control(self):
		...
		if (self.s_ev==0):
			...
			x_alin = (0.00025)*(self.v)*(1.0/30.0)*self.space_count #0.00035
			if (x_alin>=0.8):
				maneuver = 2
				self.s_ev = 0
				self.FT = 0
...
#																	LANE KEEPING AND PASSING
	def lane_keeping(self,maneuver):
		if (maneuver==2):
			# Derecha
			self.v = 325 #self.v_max #265 #self.v_max #175
			self.side = 1
			self.x_ref = 120 #118
...
#																PERPENDICULAR PARKING
	def perpendicular_parking(self):
			...
			if (self.step == 0):
				...
				v = Nv
				d_ref = 75.0*k # Separacion en [cm]
				u,d = steer_control(m,b,d_ref)
				depth = 0.5 # Profundidad del espacio para estacionarse [m]
				flag_space = lidar_roi_f(self.R,self.side, depth)

				if (flag_space==False): self.car_flag = True
				


				if (flag_space==True) and (abs(abs(d_ref)-abs(d))<=5.0) and (self.car_flag==True): self.Pflag =True
				if (self.Pflag== True): self.space_count = self.space_count+1
				x_alin = (0.065)*(1.0/30.0)*self.space_count #0.065
				#print('Pflag ', self.Pflag)
				if (self.Pflag==True) and (x_alin>=0.65):
			...
			if (self.step == 1):
				...
				u = 1.0*k
				v = -Nv #*0.85
				if (abs(self.Dyaw)>=90.0) or (r180<=0.3): self.step = self.step + 1
