...
#**************************************************************************
#**************************************************************************
# 				OBTENCION DE LOS GRUPOS DEL ARCHIVO
def get_all_groups(hdf5_file, current_path=''):
    """
    Returns a list of all groups in an HDF5 file as text strings.
    """
    groups = []
    for key in hdf5_file.keys():
        item = hdf5_file.get(key)
        path = current_path + '/' + key if current_path else key
        if isinstance(item, h5py.Group):
            groups.append(path)
            groups.extend(get_all_groups(item, path))
    return groups
#**************************************************************************
#**************************************************************************
#**************************************************************************
# 				OBTENCION DE LOS PESOS Y SESGOS
def get_KBW(h5_path):
	...
	groups = get_all_groups(hf)
	print(groups)
	#bd = np.array(hf[groups[1]+'/bias:0'])
	#wd = np.array(hf[groups[1]+'/kernel:0'])	
	#print(hf['model_weights']['dense_1']['dense_1'].keys())
	#print(list(hf))
	#print(list(hf['model_weights']))
	#print(list(hf['model_weights/dense/dense']))
	...
	for i in range (int(Nd)):
		bd = np.array(hf[groups[2*i+1]+'/bias:0'])
		wd = np.array(hf[groups[2*i+1]+'/kernel:0'])
		Bd.append(bd)
		Wd.append(wd)
