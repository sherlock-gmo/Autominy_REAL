...
from lib_lidar_functions import lidar_roi_f, lidar4ransac, lidar_ev, lidar_roi_f3     #,vis_lidar
...
#																PERPENDICULAR PARKING
	def perpendicular_parking(self):
			...
			if (self.step == 0):
				...
				v = Nv
				d_ref = 85.0*k # Separacion en [cm]
				u,d = steer_control(m,b,d_ref)
				if (abs(m)<=0.10) and (abs(d_ref-d)<=0.05):
					self.stop_rs = True
				...
				depth = 0.7 # Profundidad del espacio para estacionarse [m]
				flag_space1 = lidar_roi_f3(self.R,self.side,depth,0.25,-0.15)
				flag_space2 = lidar_roi_f3(self.R,self.side,depth,0.55,-0.2)
				#flag_space3 = lidar_roi_f3(self.R,self.side,depth,)
				#print(flag_space)
				if (flag_space1 == False) and (self.car1==False):
					self.car1 = True
					#self.stop_car_p()
				if (self.car1== True) and (flag_space2==True):
			...
			if (self.step == 1):
				...
				u = 1.0*k
				v = -Nv*1.10
				if (abs(self.Dyaw)>=90.0) or (r180<=0.28):
					v = 0.0
					self.step = self.step + 1
