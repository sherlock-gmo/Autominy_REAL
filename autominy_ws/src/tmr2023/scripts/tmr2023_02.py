...
from lib_lidar_functions import lidar_roi, lidar_roi2, lidar_side, lidar_side2, lidar_ev
...
#																PERPENDICULAR PARKING
	def perpendicular_parking(self):
			...
			if (self.step == 0):
				ymax = 1.75 #[m]
				ymin = 0.35 #0.35
				R_side = lidar_side2(self.R,ymax,ymin,self.side) # Puntos del lidar usados para el RANSAC
				m,b = fit_ransac(R_side,3.0)
				#print('m ',m)
				v = Nv
				d_ref = 75.0*k # Separacion en [cm]
				u,d = steer_control(m,b,d_ref)
				flag_space, count_side = lidar_roi2(self.R,self.side)
